...
let base_info = async function(url: string){
	...
	let xpath = '/html/body/div[@id="wrapper"]/div[@class="box_con"][1]/div[@id="maininfo"]/div[@id="info"]';
	// xpath = '/html/body/div[@id="wrapper"]/div[@class="box_con"][1]/div[@id="maininfo"]/div[@id="info"]';
	let parser = Spider.to_node(res);
    parser = Spider.xpath_search(parser, xpath);
	let buff_node_data = parser;
	// console.log('parser', parser, parser.length);
...
let all_info = async function (url:string) {
	...
	xpath = '/html/body/div[@id="wrapper"]/div[@class="box_con"][2]/div[@id="list"]/dl';
	parser = Spider.xpath_search(parser, xpath);
	// console.log(parser);
	...
	for(let index in content_node_data[0].children){
		...
		if(Spider.to_text(content_node).includes('正文')){
			content_point = true;
			continue;
		}
		if(content_point){
...
let page_read = async function(url:string){
	...
	let xpath = '/html/body/div[@id="wrapper"]/div[@class="content_read"]/div[@class="box_con"]';
	let parser = Spider.to_node(res);
	let root_node_data = parser;
	...
	parser = Spider.xpath_search(parser, '/div[@class="bookname"]/h1')[0];
	// console.log(parser);
...
// let test = async function(){
	...
	// let info = await index_all_info(65451);

	// console.log(info);

// 	let res = await page_read('/book/65451/6062608.html');
// 	console.log(res);
// }
